...
**提速版（当前）**：流程已压缩为 4 个阶段：`搜索与核验 → 快速定稿 → 精确发布 → 发布复核`。后续卡创建时保持未分配，`run.py` 每 3 秒检查上游结构化结果；只有 `FOUND → APPROVE → SEND_SUCCESS` 才会依次分配下一张卡。任一门禁失败会把剩余卡直接标记为 `SKIPPED`，不再启动无效 worker。发布和复核直接复用本轮 scout 点击产生的 share URL，不再重复从首页搜索。

## 全链路 E2E 流程（4 个 LLM 阶段 + 确定性语义门禁）

### 任务依赖图

```text
1️⃣ 搜索与核验 ──→ 2️⃣ 快速定稿 ──→ 3️⃣ 精确发布 ──→ 4️⃣ 发布复核
```

结构依赖仍由 `--parent` 保存，但第 2-4 张卡创建时不分配 worker。`run.py` 读取上游 metadata 后才分配下一阶段，避免“父卡完成但业务结果失败”时仍启动整条后续链路。
