# 发布员 Worker v0.2（真实发送，零 Vision、零常规截图）

目标：走真实用户路径绑定指定评论楼层，把参数文件中的逐字定稿输入一次，验证回复对象、文本完整性和发送按钮状态，然后**点击发送**。全程使用 snapshot + read；证据不足时安全退出，不调用 Vision，不保存像素截图。
...
- 当 a11y 已找到目标作者但评论区不在 viewport 时，先对目标作者 ref 执行一次 `agent-browser scrollintoview @<目标作者ref>`，wait 500，再 fresh snapshot。`scrollintoview` 是此状态的唯一首选动作。
- 只有 `scrollintoview` 失败或 ref stale 时，才 fresh snapshot 取得新作者 ref 后原样重试一次。禁止截图、Vision，也禁止用窗口级 scroll 代替定位。
...
- `read` 输出若不包含目标正文，或点击后不包含 `回复 <目标作者>`，立即 `NEEDS_VERIFIER`；禁止继续输入或发送。
- 当 `read` 已确认目标楼层是从上到下第 N 个同作者一级评论后，按 snapshot 中同作者一级楼层的同一顺序绑定 action row。若该行明确是两个连续 generic 数字，顺序固定解释为“点赞数 → 回复气泡数”，点击第二个；若是数字后跟 `generic "回复"`，点击文字回复。
...
- `read` 失败、正文缺失、同作者多楼层无法区分，或楼层边界不明确时，返回 `NEEDS_VERIFIER`，不点击、不输入、不发送。
...
- 纯数字只有在同时满足下列全部条件时才可作为 `REPLY_REF`：
  ...
  5. snapshot 结构明确显示该行是点赞动作 + 回复气泡动作。
...
- 若 a11y 不显示回复对象，执行 fresh `agent-browser read`，检查是否明确出现 `回复 <目标作者>` 与目标评论前缀。对象错误则 `WRONG_REPLY_TARGET`；缺失或不清楚则 `NEEDS_VERIFIER`。
...
- Vision 配额固定为 0。不可逆发送只能由 fresh snapshot + read 的明确文本和结构证据解锁；任何关键字段不明确都 fail-closed。
...
3. 回复对象必须在输入前已由 fresh read 明确为目标作者；输入后没有点击其他楼层时记录 `READ_PRECHECK_CARRIED_FORWARD`。
...
2. `get url` 确认页面未发生导航；
3. 不保存像素截图；发送成功只由发送前机械门禁、发送后 fresh snapshot 的编辑器重置，以及 URL 未异常变化共同判定。
...
`vision_calls` 固定为 `0`，`vision_format_retries` 固定为 `0`，`evidence_files` 固定为 `NONE`。
